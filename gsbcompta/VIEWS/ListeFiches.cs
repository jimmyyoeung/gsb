...
namespace gsbcompta
{
    public partial class ListeFiches : Form
    {
        ...
        private void dataGridViewOffPrice_SelectionChanged(object sender, EventArgs e)
        {
            ...
            {
                DetailsOffPrice OPForm = new DetailsOffPrice(row.Cells[0].Value.ToString(), row.Cells[5].Value.ToString(),row.Cells[4].Value.ToString(), row.Cells[3].Value.ToString(), row.Cells[2].Value.ToString(), row.Cells[1].Value.ToString());
                OPForm.Show();
                this.Close();
            }
        }
